...
SELECT 'PD250929013' ,'2'
UNION ALL 
SELECT 'PD250929014' ,'2'
UNION ALL 
SELECT 'PD250604011' ,'5'
...
--  SELECT '1'
  ...
  -- 재고 체크 
  CASE WHEN EXISTS(SELECT *FROM REAL_MES.FLEXMES.DBO.ST_STOCK_NOW A WITH (NOLOCK) 
  ...
  (
    ...
    FROM (
        ...
        LEFT JOIN REAL_MES.FLEXMES.DBO.QC_IQC_ORDER F WITH (NOLOCK) ON A.DIV_CD = F.DIV_CD 
        AND A.PLANT_CD = F.PLANT_CD AND A.ITEM_CD = F.ITEM_CD AND A.LOT_NO = F.LOT_NO 

        LEFT JOIN REAL_MES.FLEXMES.DBO.QC_PQC_ORDER G WITH (NOLOCK) ON A.DIV_CD =G.DIV_CD 
        AND A.PLANT_CD = G.PLANT_CD AND A.ITEM_CD = G.ITEM_CD AND A.LOT_NO = G.LOT_NO

        LEFT JOIN REAL_MES.FLEXMES.DBO.VIEW_VALID_DATE H WITH (NOLOCK) ON A.ITEM_CD = H.MATNR AND A.LOT_NO = H.ZVLOT 
...
BEGIN
  ...
  (
    SELECT @NOW_ORDER AS ORDER_NO, 0 AS PLAN_SEQ, ISNULL(D.QC_RESULT, E.QC_RESULT) AS QC_RESULT, '' CHG_RSN, 
      ...
      LEFT JOIN REAL_MES.FLEXMES.DBO.QC_IQC_ORDER D WITH (NOLOCK) ON A.DIV_CD = D.DIV_CD AND A.PLANT_CD = D.PLANT_CD AND A.ITEM_CD = D.ITEM_CD 
      AND A.LOT_NO = D.LOT_NO 
      LEFT JOIN REAL_MES.FLEXMES.DBO.QC_PQC_ORDER E WITH (NOLOCK) ON A.DIV_CD = E.DIV_CD AND A.PLANT_CD = E.PLANT_CD AND A.ITEM_CD = E.ITEM_CD 
