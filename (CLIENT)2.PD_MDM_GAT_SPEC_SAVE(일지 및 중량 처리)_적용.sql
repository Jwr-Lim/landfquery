...
*/  
ALTER PROC USP_PD_GAT_SPEC_SAVE(  
...
IF OBJECT_ID('tempdb..#FIFO_TABLE') IS NOT NULL
DROP TABLE #FIFO_TABLE;
...
DECLARE @LOT_CHK NVARCHAR(1) = 'N' -- LOT 증가를 위한 처리 
...
CREATE INDEX IX_FIFO_ROW ON #FIFO_TABLE(ROWNUM);
CREATE INDEX IX_FIFO_PROC_ITEM ON #FIFO_TABLE(PROC_CD, ITEM_CD);
    ...
    BEGIN  
        ...
        BEGIN  
            ...
            BEGIN   
                ...
                IF EXISTS(SELECT *FROM PD_MDM_RESULT_GROUP A WITH (NOLOCK)   WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD   
                AND A.ORDER_NO = @ORDER_NO AND A.REVISION = @REVISION AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD AND A.PROC_CD = @PROC_CD AND A.EDATE IS NULL   
                )  
                BEGIN  
                    INSERT INTO @EQP_TBL   
                ...
                BEGIN  
                    ...
                    IF @PROC_CD NOT IN ('RB')  -- 리본 믹서는 제외 하자.. 나중에 다시 확인 필요ㅜㅜ 
                    BEGIN 
                        ...
                        FROM RANKEDDATA  
                        WHERE RN = 1  
                          AND PROC_CD <> @PROC_CD AND DEPARTMENT NOT IN 
                            (SELECT EQP_CD FROM BA_EQP WHERE DIV_CD = @DIV_CD AND PLANT_CD = @PLANT_CD AND WC_CD = @WC_CD AND LINE_CD = @LINE_CD  
                             AND PROC_CD = @PROC_CD AND EQP_CD <> @EQP_CD AND ISNULL(MN_FLAG,'') <> 'Y' AND TP <> '' 
                             ) 
...
--                    SELECT '1'  
                ...
                IF @MN_S IN ('Y')  
                    ...
                    BEGIN  
                        ...
                        WHERE A.PLANT_CD = @PLANT_CD  
                            ...
                            -- 어... 여기는..                                   
                            -- 채번 규칙을 체인지 합시다..                                   
                            --SELECT CASE WHEN @LOT_CHK = 'N' THEN 1 ELSE 0 END 
 
                            -- 그룹공정에 있고, 순번이 1 이면 증가고, 1이 아니면? 증가를 하면 안된다. 그대로 가지고 간다.  
                            -- 나중에는 리본에서 그대로 가지고 내려오도록 해야 된다.  
 
                            IF EXISTS(SELECT  
                                *FROM PD_MDM_RESULT_GROUP A WITH (NOLOCK)  
                            WHERE A.PD_AUTO_NO = @PD_AUTO_NO AND A.ORDER_NO = @ORDER_NO AND A.REVISION = @REVISION  
                            AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD AND A.PROC_CD = @PROC_CD AND A.RESULT_SEQ = @RESULT_SEQ 
                            AND A.EDATE IS NULL) 
                            BEGIN 
                                SET @LOT_CHK = 'N' 
                            END  
                            -- @LOT CHK 가 N 이면 증가고 아니면 예전 채번 그대로 가지고 간다.                                   
            ...
            BEGIN  
                ...
                FROM PD_MDM_RESULT_GROUP A WITH (NOLOCK)   
                INNER JOIN PD_RESULT B WITH (NOLOCK)   ON A.DIV_CD = B.DIV_CD AND A.PLANT_CD = B.PLANT_CD AND A.ORDER_NO = B.ORDER_NO AND A.REVISION = B.REVISION   
