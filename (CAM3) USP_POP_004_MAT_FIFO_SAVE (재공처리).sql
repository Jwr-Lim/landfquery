...
*/                                    
        ...
        BEGIN 
            ...
            SELECT B.PROC_CD, B.PROC_SEQ  FROM PD_ORDER_PROC A  
            INNER JOIN PD_ORDER_PROC B  
            ON                                      
            ...
            AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD AND A.PROC_SEQ > B.PROC_SEQ AND B.OUT_CHK = 'Y' AND B.SKIP = 'N'  -- 24.08.12 LJW 추가 SKIP 부분                
    ...
    -- 이게 중요 할것 같음... 이 쿼리가 변경이 될수도 있는가? 확인할것..                                      
      ...
      B.QTY +                                      
        ISNULL((SELECT SUM(AA.QTY) FROM ST_ITEM_IN AA  
    	    ...
    	    ISNULL((SELECT SUM(AA.QTY) FROM ST_ITEM_OUT AA  
    	    WHERE AA.DIV_CD = A.DIV_CD AND AA.PLANT_CD = A.PLANT_CD AND AA.ITEM_CD = A.ITEM_CD AND AA.LOT_NO = A.LOT_NO AND AA.PROC_CD = A.PROC_CD AND  AA.LOCATION_NO = A.LOCATION_NO                                      
              AND AA.ORDER_NO = B.ORDER_NO AND AA.ORDER_SEQ = B.ORDER_SEQ AND AA.RESULTS_SEQ = B.RESULTS_SEQ                                      
        ...
        INNER JOIN V_ITEM B1 ON B.PLANT_CD = B1.PLANT_CD AND  B.ITEM_CD = B1.ITEM_CD                                      
        INNER JOIN BA_SUB_CD B2 ON B1.BASE_ITEM_CD = B2.SUB_CD AND B2.MAIN_CD = 'BA206'                                     
...
*/                                    
            ...
            SELECT                                      
                 ...
                 @ORDER_TYPE,         @ORDER_FORM,           @ROUT_NO,                @ROUT_VER,               @WC_CD,                                      
                 @LINE_CD,            @PROC_CD,              @RESULT_SEQ,                                                  
                 ISNULL((SELECT MAX(Z.USEM_SEQ) FROM PD_USEM Z  
            ...
            BEGIN                                      
                ...
                BEGIN                                      
                    ...
                    D.RESULTS_SEQ AS RESULT_SEQ                                     
                      ...
                      INNER JOIN PD_ORDER B ON A.DIV_CD = B.DIV_CD AND A.PLANT_CD = B.PLANT_CD AND A.ORDER_NO = B.ORDER_NO AND A.REVISION = B.REVISION AND A.PROC_NO = B.PROC_NO                                      
                      ...
                      INNER JOIN PD_ORDER_PROC B1 ON B.DIV_CD = B1.DIV_CD AND B.PLANT_CD = B1.PLANT_CD AND B.PROC_NO = B1.PROC_NO                                                        
                      INNER JOIN ST_STOCK_NOW C ON B1.DIV_CD = C.DIV_CD AND B1.PLANT_CD = C.PLANT_CD AND C.SL_CD = '3000'                                      
                      ...
                      */            
                      INNER JOIN V_ITEM E ON B.PLANT_CD = E.PLANT_CD AND C.ITEM_CD = E.ITEM_CD AND E.REP_ITEM_CD = @GROUP_ITEM                                     
                      INNER JOIN BA_SUB_CD F ON E.BASE_ITEM_CD = F.SUB_CD AND F.MAIN_CD = 'BA206'                                     
...
--        SELECT @SIL_PROC_CD                    
        ...
        BEGIN           
          ...
          FROM ST_STOCK_NOW A  
              ...
              INNER JOIN V_ITEM B1 ON B.PLANT_CD = B1.PLANT_CD AND  B.ITEM_CD = B1.ITEM_CD                  
              INNER JOIN BA_SUB_CD B2 ON B1.BASE_ITEM_CD = B2.SUB_CD AND B2.MAIN_CD = 'BA206'                                     
              LEFT JOIN PD_RESULT C ON B.DIV_CD = C.DIV_CD AND B.PLANT_CD = C.PLANT_CD AND B.WC_CD = C.WC_CD AND B.LOCATION_NO = C.LINE_CD AND B.PROC_CD = C.PROC_CD                                     
...
--      SELECT @BE_WC_CD, @SIL_PROC_CD, @SIL_LINE_CD                                      
        ...
        BEGIN                                      
            ...
            FROM PD_RESULT A  
            INNER JOIN PD_ORDER_PROC A1 ON                           
            A.DIV_CD = A1.DIV_CD AND A.PLANT_CD = A1.PLANT_CD AND A.ORDER_NO = A1.ORDER_NO AND A.REVISION = A1.REVISION                           
            AND A.ORDER_TYPE = A1.ORDER_TYPE AND A.ORDER_FORM = A1.ORDER_FORM AND A.ROUT_NO = A1.ROUT_NO AND A.ROUT_VER = A1.ROUT_VER                           
            AND A.WC_CD = A1.WC_CD AND A.LINE_CD = A1.LINE_CD AND A.PROC_CD = A1.PROC_CD                          
                          
            INNER JOIN PD_RESULT B ON A.DIV_CD = B.DIV_CD AND A.PLANT_CD = B.PLANT_CD AND A.ORDER_NO = B.ORDER_NO AND A.REVISION = B.REVISION AND A.ORDER_TYPE = B.ORDER_TYPE                                     
            AND A.ORDER_FORM = B.ORDER_FORM AND A.ROUT_NO = B.ROUT_NO AND A.ROUT_VER = B.ROUT_VER AND A.WC_CD = B.WC_CD AND A.LINE_CD = B.LINE_CD --AND A.PROC_CD <> B.PROC_CD                       
            AND B.S_CHK = 'N' AND A.RESULT_SEQ = B.RESULT_SEQ AND A.GROUP_LOT = B.GROUP_LOT                                      
            INNER JOIN PD_ORDER_PROC B1 ON                           
            B.DIV_CD = B1.DIV_CD AND B.PLANT_CD = B1.PLANT_CD AND B.ORDER_NO = B1.ORDER_NO AND B.REVISION = B1.REVISION                           
            AND B.ORDER_TYPE = B1.ORDER_TYPE AND B.ORDER_FORM = B1.ORDER_FORM AND B.ROUT_NO = B1.ROUT_NO AND B.ROUT_VER = B1.ROUT_VER                           
            AND B.WC_CD = B1.WC_CD AND B.LINE_CD = B1.LINE_CD AND B.PROC_CD = B1.PROC_CD AND A1.GROUP_SEQ >= B1.GROUP_SEQ                           
                          
            INNER JOIN PD_USEM C ON B.DIV_CD = C.DIV_CD AND B.PLANT_CD = C.PLANT_CD AND B.ORDER_NO = C.ORDER_NO AND B.REVISION = C.REVISION AND B.ORDER_TYPE = C.ORDER_TYPE                              
            AND B.ORDER_FORM  = C.ORDER_FORM AND B.ROUT_NO = C.ROUT_NO AND B.ROUT_VER = C.ROUT_VER AND B.WC_CD = C.WC_CD AND B.LINE_CD = C.LINE_CD AND B.PROC_CD = C.PROC_CD AND                                      
            C.IN_GBN IN ('SU') AND C.USE_CHK = 'N' AND B.RESULT_SEQ = C.RESULT_SEQ                                      
        ...
        SELECT B.ORDER_NO, B.REVISION, B.ROUT_NO, B.ROUT_VER, B.WC_CD, B.LINE_CD, B.PROC_CD, B.RESULT_SEQ, B.GROUP_LOT, ISNULL(D.SPECIPI_YN,'N'), ISNULL(C.AP_CHK,'N')                                    
           ...
           INNER JOIN PD_ORDER_PROC C ON B.DIV_CD = C.DIV_CD AND B.PLANT_CD = C.PLANT_CD AND B.ORDER_NO = C.ORDER_NO                                      
            ...
            LEFT JOIN BA_ROUTING_DETAIL D ON C.DIV_CD = D.DIV_CD AND C.PLANT_CD = D.PLANT_CD AND C.ROUT_NO = D.ROUT_NO AND C.ROUT_VER = D.[VERSION] AND C.PROC_CD = D.PROC_CD                                      
            ...
            BEGIN                       
              ...
              */                           
              ...
              BEGIN                                   
                ...
                CAST(SUBSTRING(@LINE_CD,4,3) AS NVARCHAR) END + '-' + dbo.LPAD(A.LOT_SEQ +                                      
...
/*(                                      
              ...
              BEGIN                   
                ...
                CAST(SUBSTRING(@LINE_CD,4,3) AS NVARCHAR) END + '-' + dbo.LPAD(1,3,0)       
...
--   SELECT @MSG_DETAIL           
            ...
            -- 마지막 공정이라면 
            -- 해당 LOT 로 바코드 생성을 진행해야 된다.
            -- 진행하면서 SAP 도 전송을 해야 한다. 
