...
DELETE A FROM MT_ITEM_OUT_BATCH A   
WHERE A.ORDER_NO IN (SELECT ORDER_NO FROM @ORDER_TBL)   
   
INSERT INTO MT_ITEM_OUT_BATCH    
SELECT *FROM REAL_MES.FLEXMES.DBO.MT_ITEM_OUT_BATCH A   
WHERE A.ORDER_NO IN (SELECT ORDER_NO FROM @ORDER_TBL)   
*/   
/*   
1. 배정이냐 아니냐를 판단한다.   
...
BEGIN    
  ...
  SELECT TOP 1 @PROC_ORDER = A.ORDER_NO, @PROC_ORDER_REV = A.REVISION, @PROC_GROUP = A.PROC_NO, @SDATE = A.SDATE 
  ...
  SELECT TOP 1 @BE_PROC_ORDER = A.ORDER_NO, @BE_PROC_ORDER_REV = A.REVISION, @BE_PROC_GROUP = A.PROC_NO, @BE_SDATE = A.SDATE 
  FROM PD_RESULT A WITH (NOLOCK)    
...
--  SELECT @SDATE, @BE_SDATE   
  IF DATEADD(DAY,-1,ISNULL(@SDATE,GETDATE())) < @BE_SDATE    
  BEGIN   
    -- 안에 있으면?    
    ...
    IF ISNULL(@PROC_ORDER,'') <> @BE_PROC_ORDER AND ISNULL(@PROC_GROUP,'') <> @BE_PROC_GROUP   
    BEGIN    
...
BEGIN    
  ...
  IF EXISTS(   
    SELECT *FROM FLEXMES.DBO.MT_ITEM_OUT_BATCH A WITH (NOLOCK)    
    WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.REQ_DT = @REQ_DT AND A.WC_CD = @WC_CD AND A.LINE_CD =@LINE_CD    
      AND A.USE_FLG <> 'N'    
  )   
  BEGIN    
 
    SELECT TOP 1 @NOW_ORDER = A.ORDER_NO FROM FLEXMES.DBO.MT_ITEM_OUT_BATCH A WITH (NOLOCK)    
    WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.REQ_DT = @REQ_DT AND A.WC_CD = @WC_CD AND A.LINE_CD =@LINE_CD    
      AND A.USE_FLG <> 'N' AND ISNULL(A.ORDER_NO, '') <> '' 
...
SELECT A.ITEM_CD FROM FLEXMES.DBO.PD_ORDER_USEM A WITH (NOLOCK)    
...
--  SELECT '1'   
  ...
  BBB.ITEM_NM,AAA.LOT_NO, AAA.REQ_QTY,    
  CASE WHEN EXISTS(   
    SELECT *FROM MT_ITEM_OUT_BATCH_ABB WHERE DIV_CD = AAA.DIV_cD AND PLANT_CD = AAA.PLANT_CD AND REQ_NO = AAA.REQ_NO    
    AND LOT_NO = AAA.LOT_NO    
  ) THEN 'S' ELSE    
  AAA.USE_FLG END USE_FLG,    
  AAA.REQ_NO, AAA.PLAN_SEQ AS P_SEQ, AAA.BATCH_NO,   
  -- 재고 체크    
  CASE WHEN EXISTS(SELECT *FROM FLEXMES.DBO.ST_STOCK_NOW A WITH (NOLOCK)    
  ...
  (   
    ...
    FROM (   
        ...
        INNER JOIN    
        FLEXMES.DBO.PD_ORDER_USEM B WITH (NOLOCK) ON A.DIV_CD = B.DIV_CD AND A.PLANT_CD = B.PLANT_CD    
        ...
        LEFT JOIN FLEXMES.DBO.QC_IQC_ORDER F WITH (NOLOCK) ON A.DIV_CD = F.DIV_CD    
        AND A.PLANT_CD = F.PLANT_CD AND A.ITEM_CD = F.ITEM_CD AND A.LOT_NO = F.LOT_NO    
   
        LEFT JOIN FLEXMES.DBO.QC_PQC_ORDER G WITH (NOLOCK) ON A.DIV_CD =G.DIV_CD    
        AND A.PLANT_CD = G.PLANT_CD AND A.ITEM_CD = G.ITEM_CD AND A.LOT_NO = G.LOT_NO   
   
        LEFT JOIN FLEXMES.DBO.VIEW_VALID_DATE H WITH (NOLOCK) ON A.ITEM_CD = H.MATNR AND A.LOT_NO = H.ZVLOT    
...
BEGIN   
  ...
  (   
    SELECT @NOW_ORDER AS ORDER_NO, 0 AS PLAN_SEQ, ISNULL(D.QC_RESULT, E.QC_RESULT) AS QC_RESULT, '' CHG_RSN,    
      ...
      LEFT JOIN FLEXMES.DBO.QC_IQC_ORDER D WITH (NOLOCK) ON A.DIV_CD = D.DIV_CD AND A.PLANT_CD = D.PLANT_CD AND A.ITEM_CD = D.ITEM_CD    
      AND A.LOT_NO = D.LOT_NO    
      LEFT JOIN FLEXMES.DBO.QC_PQC_ORDER E WITH (NOLOCK) ON A.DIV_CD = E.DIV_CD AND A.PLANT_CD = E.PLANT_CD AND A.ITEM_CD = E.ITEM_CD    
  ...
  LEFT JOIN BA_SUB_CD BBB WITH (NOLOCK) ON AAA.QC_RESULT = BBB.SUB_CD AND BBB.MAIN_CD = 'SAP08'   
