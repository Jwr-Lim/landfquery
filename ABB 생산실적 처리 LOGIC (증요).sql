        ...
        BEGIN 
            ...
            BEGIN 
                ...
                BEGIN  
                    ...
                    CASE WHEN @MN_S = 'Y' THEN 1 ELSE 0 END 
...
--                    SELECT *FROM @RESULT_PROC 
                ...
                --        EXEC USP_CM_AUTO_NUMBERING 'PR', @STR_DATE, @USER_ID, @LOT_NO OUT                 
                        ...
                        SET @LOT_SEQ = ISNULL((SELECT MAX(A.LOT_SEQ) FROM PD_RESULT A WITH (NOLOCK) 
                        WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.PROC_NO = @PROC_NO AND A.ORDER_NO = @ORDER_NO 
                          AND A.REVISION = @REVISION AND A.ORDER_TYPE = @ORDER_TYPE AND A.ORDER_FORM = @ORDER_FORM AND A.ROUT_NO = @ROUT_NO 
                          AND A.ROUT_VER = @ROUT_VER AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD AND A.PROC_CD = @PROC_CD AND A.S_CHK = @S_CHK 
                          AND A.EQP_CD = @EQP_CD 
                          AND A.GROUP_LOT = @LOT_NO),0) + 1

                        SET @LOT_NO = @MASTER_LOT + '-' + @LOT_SEQ 
                    ...
                    SELECT *FROM PD_RESULT A WITH (NOLOCK) WHERE A.ORDER_NO = 'PD250827001'
                    SELECT *FROM PD_USEM A WITH (NOLOCK) WHERE A.ORDER_NO = 'PD250827001'
                    SELECT *FROM PD_ITEM_IN A WITH (NOLOCK) WHERE A.ORDER_NO = 'PD250827001'
                    END 
                    -- 끝
                END 
...
A.DIV_CD = B.DIV_CD AND A.PLANT_CD = B.PLANT_CD AND A.OMT_RDER_NO = B.ORDER_NO AND A.REVISION = B.REVISION AND 
...
A.LINE_CD = B.LINE_CD AND A.S_CHK = B.S_CHK AND A.RESULT_SEQ = B.RESULT_SEQ AND B.SPEC_VALUE <> ''
