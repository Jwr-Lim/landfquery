...
ALTER PROC [dbo].[USP_POP_004_MATERIAL_START](  
--DECLARE   
...
BEGIN TRY   
    ...
    IF EXISTS(  
        SELECT *FROM FLEXMES.DBO.PD_RESULT A WITH (NOLOCK)   
    ...
    BEGIN   
        SET @MSG_CD = '0060'   
        SET @MSG_DETAIL = '중복된 Seq 의 실적이 있습니다. 시작할수 없습니다. 확인하여 주십시오.'  
  ...
  --          WHERE DIV_CD = @DIV_CD AND PLANT_CD = @PLANT_CD AND ORDER_NO = @ORDEr_NO AND REVISION = @REVISION)  
    ...
    BEGIN   
        ...
        BEGIN 
            ...
            -- PD_RESULT  
            -- PD_USEM 
            -- PD_RESULT_PROC_SPEC_VALUE  
            DECLARE @REAL_MSG_CD     NVARCHAR(4)  
                ,@REAL_MSG_DETAIL NVARCHAR(MAX)  
                ,@I_VALUE         INT  
             
            EXEC @I_VALUE = FLEXMES.DBO.USP_POP_004_WORK_START 
            @DIV_CD = @DIV_CD  
            ,@PLANT_CD = @PLANT_CD  
            ,@ORDER_NO = @ORDER_NO  
            ,@REVISION = @REVISION  
            ...
            ,@CYCLE_SEQ = 1 
            ,@USER_ID = 'MDM'  
            ,@MSG_CD = @REAL_MSG_CD               OUTPUT  
            ,@MSG_DETAIL = @REAL_MSG_DETAIL       OUTPUT  
 
            IF (@I_VALUE = 1) 
            BEGIN  
                SET @MSG_CD = @REAL_MSG_CD  
                SET @MSG_DETAIL = @REAL_MSG_DETAIL  
                RETURN 1 
            END  
 
            -- 원료 투입 
 
            INSERT INTO FLEXMES.DBO.PD_USEM   
            (  
                DIV_CD,            PLANT_CD,             PROC_NO,                ORDER_NO,                REVISION,        
                ORDER_TYPE,        ORDER_FORM,           ROUT_NO,                ROUT_VER,                WC_CD,   
                LINE_CD,           PROC_CD,              RESULT_SEQ,             USEM_SEQ,                USEM_WC,   
                USEM_PROC,         ITEM_CD,              SL_CD,                  LOCATION_NO,   
                RACK_CD,           LOT_NO,               MASTER_LOT,             BARCODE,                   
                PLC_QTY,           USEM_QTY,             USE_CHK,                DEL_FLG,                 REWORK_FLG,   
                INSERT_ID,         INSERT_DT,            UPDATE_ID,              UPDATE_DT,               REQ_DT,   
                REQ_NO,            REQ_SEQ,              PLAN_SEQ,               BATCH_NO,                BE_ORDER_NO,  
            ...
            SELECT   
                ...
                B.ORDER_TYPE,      B.ORDER_FORM,         B.ROUT_NO,              B.ROUT_VER,              B.WC_CD,   
                B.LINE_CD,         B.PROC_CD,            B.RESULT_SEQ,           B.USEM_SEQ,              B.USEM_WC,   
                B.USEM_PROC,       B.ITEM_CD,            B.SL_CD,                B.LOCATION_NO,   
                B.RACK_CD,         B.LOT_NO,             B.MASTER_LOT,           B.BARCODE,             
                B.PLC_QTY,         B.USEM_QTY,           B.USE_CHK,              B.DEL_FLG,               B.REWORK_FLG,   
                @USER_ID,          GETDATE(),            @USER_ID,               GETDATE(),               B.REQ_DT,   
                B.REQ_NO,          B.REQ_SEQ,            B.PLAN_SEQ,             B.BATCH_NO,              B.BE_ORDER_NO,   
            ...
            UPDATE AA SET AA.USE_QTY = BB.USE_QTY, AA.USE_FLG = 'N'  
            FROM FLEXMES.DBO.MT_ITEM_OUT_BATCH AA  
            INNER JOIN   
            (  
            SELECT B.DIV_CD, B.PLANT_CD, B.REQ_NO, SUM(B.USEM_QTY) AS USE_QTY FROM PD_USEM_MAT_TEMP_MASTER A WITH (NOLOCK)   
                INNER JOIN PD_USEM_MAT_TEMP B WITH (NOLOCK) ON A.DIV_cD = B.DIV_CD AND A.PLANT_CD = B.PLANT_CD AND A.ORDER_NO = B.ORDER_NO AND   
            ...
            DECLARE @MAX_REVISION INT = 0  
 
            SELECT @MAX_REVISION = MAX(A.REVISION) 
            FROM FLEXMES.DBO.PD_ORDER A WITH (NOLOCK) WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.ORDER_NO = @ORDER_NO 
 
            INSERT INTO @ORDER_TABLE  
            SELECT A.ORDER_NO, A.REVISION FROM  
            FLEXMES.DBO.PD_ORDER A WITH (NOLOCK) WHERE A.PROC_NO IN ( 
            SELECT A.PROC_NO FROM FLEXMES.DBO.PD_ORDER A WITH (NOLOCK) WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.ORDER_NO = @ORDER_NO AND A.REVISION = @MAX_REVISION  
            ) 
 
 
 
            IF NOT EXISTS(SELECT *FROM PD_ORDER A WITH (NOLOCK)  
            INNER JOIN @ORDER_TABLE B ON A.ORDER_NO = B.ORDER_NO AND A.REVISION = B.REVISION 
            ) 
            BEGIN 
             
                INSERT INTO PD_ORDER  
                SELECT A.*FROM FLEXMES.DBO.PD_ORDER A WITH (NOLOCK)  
                INNER JOIN @ORDER_TABLE B ON A.ORDER_NO = B.ORDER_NO AND A.REVISION = B.REVISION 
            END  
 
            IF NOT EXISTS(SELECT *FROM PD_ORDER_USEM A WITH (NOLOCK)  
            INNER JOIN @ORDER_TABLE B ON A.ORDER_NO = B.ORDER_NO AND A.REVISION = B.REVISION 
            ) 
            BEGIN 
                INSERT INTO PD_ORDER_USEM  
                SELECT A.*FROM FLEXMES.DBO.PD_ORDER_USEM A WITH (NOLOCK)  
                INNER JOIN @ORDER_TABLE B ON A.ORDER_NO = B.ORDER_NO AND A.REVISION = B.REVISION 
            ...
            ,A.UPDATE_ID 
            ,A.UPDATE_DT 
                FROM FLEXMES.DBO.PD_ORDER_PROC A WITH (NOLOCK)  
                INNER JOIN @ORDER_TABLE B ON A.ORDER_NO = B.ORDER_NO AND A.REVISION = B.REVISION 
         
            END  
 
            IF NOT EXISTS(SELECT *FROM PD_ORDER_PROC_SPEC A WITH (NOLOCK)  
            INNER JOIN @ORDER_TABLE B ON A.ORDER_NO = B.ORDER_NO AND A.REVISION = B.REVISION 
            ) 
            BEGIN  
                -- 라인이 어디인가?  
                ...
                SELECT @TCNT = COUNT(*) FROM @ORDER_TABLE  
 
                WHILE @CNT <> @TCNT  
                BEGIN  
                    SET @CNT = @CNT + 1  
 
                    DECLARE @L_ORDER_NO      NVARCHAR(50) = '' 
                        ,@L_REVISION      INT = 0  
                        ,@BASE_ITEM_CD    NVARCHAR(50) = '' 
                        ,@USEM_ITEM_GROUP NVARCHAR(50) = '' 
                         
 
                    SELECT @L_ORDER_NO = ORDER_NO, @L_REVISION = REVISION FROM @ORDER_TABLE WHERE CNT = @CNT  
 
 
 
                    SELECT @PROC_NO = A.PROC_NO, @ORDER_TYPE = A.ORDER_TYPE, @ORDER_FORM = A.ORDER_FORM, @ROUT_NO = A.ROUT_NO, @ROUT_VER = A.ROUT_VER, @WC_CD = A.WC_CD, @LINE_CD = A.LINE_CD, 
                    @BASE_ITEM_CD = B.BASE_ITEM_CD, @USEM_ITEM_GROUP = B.REP_ITEM_CD 
                        FROM FLEXMES.DBO.PD_ORDER A WITH (NOLOCK)  
                    ...
                    SELECT  
                    @DIV_CD, @PLANT_CD, @L_ORDER_NO, @L_REVISION, @ORDER_TYPE, @ORDER_FORM, @ROUT_NO, @ROUT_VER, @WC_CD, @LINE_CD, B.PROC_CD,  
                    ROW_NUMBER() OVER (PARTITION BY B.PROC_CD ORDER BY B.PROC_SPEC_CD),  
                    1, @BASE_ITEM_CD,  
                    '', -- MODEL 
                    '001','',B.PROC_SPEC_CD, B.EQP_CD, 1,'kg','20', CASE WHEN ISNULL(C.USEM_ITEM_GROUP,'') = '' THEN @USEM_ITEM_GROUP ELSE C.USEM_ITEM_GROUP END, 
                    0,0,'','',0,0,0,1,0,0,'','Y','N','Y','N','N','N','N','N','','','Y','Y', 'mdm',getdate(),'mdm',getdate(),NULL 
 
                    FROM BA_EQP A WITH (NOLOCK)  
                    INNER JOIN POP_EQP_ENO B WITH (NOLOCK) ON A.DIV_CD = B.DIV_CD AND A.PROC_CD = B.PROC_CD AND A.EQP_CD = B.EQP_CD AND ISNULL(B.MDM_TAG,'') <> '' 
                    LEFT JOIN FLEXMES.DBO.PD_ORDER_PROC_SPEC C WITH (NOLOCK) ON B.DIV_CD = C.DIV_cD AND A.WC_CD = C.WC_CD AND A.LINE_CD = C.LINE_CD  
                    AND B.PROC_CD = C.PROC_CD AND B.EQP_BACK = C.EQP_CD AND B.PROC_SPEC_CD = C.PROC_SPEC_CD 
                    AND C.ORDER_NO = @L_ORDER_NO AND C.REVISION = @L_REVISION   
 
                    WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD 
            /*        -- 21 개  
                    SELECT AA.* 
                    FROM FLEXMES.DBO.PD_ORDER_PROC_SPEC AA WITH (NOLOCK) 
                    INNER JOIN  
                    (SELECT B.* FROM BA_EQP A WITH (NOLOCK)  
                    INNER JOIN POP_EQP_ENO B WITH (NOLOCK) ON A.DIV_CD = B.DIV_CD AND A.PROC_CD = B.PROC_CD AND A.EQP_CD = B.EQP_CD AND ISNULL(B.MDM_TAG,'') <> '' 
                    WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD) BB   
                    ON AA.DIV_CD = BB.DIV_CD AND AA.PLANT_CD = BB.PLANT_CD AND AA.PROC_CD = BB.PROC_CD AND AA.EQP_CD = BB.EQP_BACK AND AA.PROC_SPEC_CD = BB.PROC_SPEC_CD 
                    WHERE AA.DIV_CD = @DIV_CD AND AA.PLANT_CD = @PLANT_CD AND AA.ORDEr_NO = @L_ORDER_NO AND AA.REVISION = @L_REVISION  
            */ 
