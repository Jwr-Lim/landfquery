...
BEGIN 
    ...
    DECLARE @PROC_SEQ INT = 0  
         
    SELECT @PROC_SEQ = PROC_SEQ FROM BA_PROC_MAIN WHERE WC_CD = @WC_CD AND PROC_CD = @PROC_CD  
 
 
    IF EXISTS(  
        SELECT TOP 1 *FROM PD_RESULT A WITH (NOLOCK)   
        WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD AND A.PROC_CD  = (SELECT TOP 1 PROC_CD FROM BA_PROC_MAIN WHERE WC_CD = @WC_CD AND PROC_SEQ <= @PROC_SEQ AND PROC_CD <> 'FG')  
    ...
    BEGIN   
        ...
        WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD AND A.PROC_CD  
        = (SELECT TOP 1 PROC_CD FROM BA_PROC_MAIN WHERE WC_CD = @WC_CD AND PROC_SEQ <= @PROC_SEQ AND PROC_CD <> 'FG') 
          AND A.ORDER_NO <> '' 
        ORDER BY A.SDATE DESC   
...
(  
    ...
    WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD --AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD   
     AND A.ORDER_NO NOT IN ('PD250113003' ,'PD250113004','PD250113005') 
