...
SELECT A.PROC_CD, B.PROC_NM, C.EQP_CD, C.EQP_NM, A.PROC_SEQ
...
SELECT A.PROC_CD, A.PROC_NM, MAX(A.PROC_SEQ) AS LAST_PROC_SEQ FROM @LAST_DATA A 
GROUP BY A.PROC_CD, A.PROC_NM 
ORDER BY LAST_PROC_SEQ

SELECT A.* FROM @LAST_DATA A 
