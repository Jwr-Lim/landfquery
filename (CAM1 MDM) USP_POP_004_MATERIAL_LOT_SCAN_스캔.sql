...
*/    
    ...
    begin     
        ...
        FROM     
            (    
            SELECT MAX(A.RESULT_SEQ) AS SEQ FROM FLEXMES.DBO.PD_RESULT A WITH (NOLOCK)     
            WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.ORDER_NO = @ORDER_NO AND A.REVISION = @REVISION     
    ...
    BEGIN     
        ...
        INNER JOIN FLEXMES.DBO.PD_ORDER_USEM B WITH (NOLOCK) ON A.DIV_CD = B.DIV_CD AND A.PLANT_CD = B.PLANT_CD AND A.ORDER_NO = B.ORDER_NO AND A.ITEM_CD = B.ITEM_CD AND B.EQP_CD = @EQP_CD     
        AND B.REVISION = (SELECT MAX(REVISION) FROM FLEXMES.DBO.PD_ORDER WHERE ORDER_NO = A.ORDER_NO)  
...
--    SELECT @REP_ITEM_CD     
    ...
    BEGIN     
        ...
        SELECT @LOT_NO = A.LOT_NO, @ITEM_CD = A.ITEM_CD FROM FLEXMES.DBO.ST_sTOCK_NOW A WITH (NOLOCK)     
...
--            SELECT @MSG_CD, @MSG_DETAIL --MSG    
    ...
    IF @BARCODE = '' AND @LOT_NO <> ''     
    BEGIN     
        SELECT TOP 1 @BARCODE = A.BARCODE, @ITEM_CD = A.ITEM_CD FROM FLEXMES.DBO.ST_STOCK_NOW A WITH (NOLOCK)     
        INNER JOIN FLEXMES.DBO.PALLET_MASTER B WITH (NOLOCK) ON A.DIV_CD = B.DIV_CD AND A.ITEM_CD = B.ITEM_CD AND A.LOT_NO = B.LOT_NO AND A.BARCODE = B.BARCODE     
...
--    SELECT @BARCODE, @LOT_NO, @ITEM_CD -- DCHK    
    ...
    IF EXISTS(SELECT *FROM FLEXMES.DBO.ST_STOCK_NOW A WITH (NOLOCK)     
    ...
    BEGIN     
        -- 수량을 가지고 옵시다.    
        SELECT @STOCK_QTY = A.QTY FROM FLEXMES.DBO.ST_STOCK_NOW A WITH (NOLOCK)     
        WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.PROC_CD = '*' AND A.ITEM_CD = @ITEM_CD AND A.LOT_NO = @LOT_NO AND A.BARCODE = @BARCODE AND A.LOT_NO = @LOT_NO     
        AND A.SL_CD = '3000' AND A.RACK_CD <> '*'    
    END     
    ...
    IF ISNULL((SELECT B.BASE_ITEM_CD FROM FLEXMES.DBO.PD_ORDER A WITH (NOLOCK)        
    ...
    BEGIN              
        ...
        BEGIN           
            ...
            (          
            SELECT TOP 1 A.QC_RESULT, A.ORDER_DT AS DT FROM FLEXMES.DBO.QC_IQC_ORDER A WITH (NOLOCK)              
                WHERE A.ITEM_CD = @ITEM_CD AND A.LOT_NO = @LOT_NO              
            UNION ALL           
            SELECT TOP 1 A.QC_RESULT, A.SIL_DT AS DT FROM FLEXMES.DBO.QC_PQC_ORDER A WITH (NOLOCK)              
                WHERE A.ITEM_CD = @ITEM_CD AND A.LOT_NO = @LOT_NO             
            ) AA          
            ORDER BY AA.DT DESC),'')         
        END           
...
--        SELECT @MSG_CD, @MSG_DETAIL --MSG    
    ...
    BEGIN         
        ...
        */                    
        IF ISNULL((         
            SELECT TOP 1 A.ZEFLK FROM FLEXMES.DBO.SAP_Z02MESF_D080 A WITH (NOLOCK)          
        ...
        BEGIN          
            ...
            IF EXISTS(SELECT *FROM FLEXMES.DBO.VIEW_VALID_DATE A WITH (NOLOCK)          
            WHERE A.MATNR = @ITEM_CD AND A.ZVLOT = @LOT_NO)         
            BEGIN          
                
                IF ISNULL((SELECT TOP 1 A.VFDAT FROM FLEXMES.DBO.VIEW_VALID_DATE A WITH (NOLOCK)          
                ...
                BEGIN          
                    ...
                    + '투입일 : ' + @SIL_DT + CHAR(10)          
                    + '유효일 : ' + CONVERT(NVARCHAR(10), CAST((SELECT TOP 1 ISNULL(A.VFDAT, '') FROM FLEXMES.DBO.VIEW_VALID_DATE A WITH (NOLOCK)          
...
--                SELECT @MSG_CD, @MSG_DETAIL --MSG         
    ...
    --2-2. 현재 들어온 작업지시 및 LOT 가 순서에 맞는가 부터 확인한다.    
        ...
        SELECT TOP 1 @NOW_REQ_NO = A.REQ_NO, @NOW_BATCH_STR = RIGHT(A.REQ_DT, 2) + '-' + CAST(A.PLAN_SEQ AS NVARCHAR) FROM FLEXMES.DBO.MT_ITEM_OUT_BATCH A WITH (NOLOCK)     
        ...
        SELECT TOP 1 @BE_REQ_NO = A.REQ_NO, @BE_BATCH_STR = RIGHT(A.REQ_DT, 2) + '-' + CAST(A.PLAN_SEQ AS NVARCHAR) FROM FLEXMES.DBO.MT_ITEM_OUT_BATCH A WITH (NOLOCK)     
...
--            SELECT @MSG_CD, @MSG_DETAIL --MSG    
    ...
    SELECT @USEM_WC = A.WC_CD, @USEM_PROC = A.PROC_CD, @USEM_LOCATION = A.LOCATION_NO, @RACK_CD = A.RACK_CD    
        FROM FLEXMES.DBO.ST_STOCK_NOW A WITH (NOLOCK)    
    ...
    SELECT @STANDARD_DATE = A.VFDAT FROM FLEXMES.DBO.VIEW_VALID_DATE A WITH (NOLOCK)     
    WHERE A.MATNR = @ITEM_CD AND A.ZVLOT = @LOT_NO    
...
IF @BATCH_CHK = 'Y'    
BEGIN     
    INSERT INTO MT_ITEM_OUT_BATCH_ABB    
    SELECT @DIV_CD, @PLANT_CD, @REQ_NO, @LOT_NO, 'admin',GETDATE()   
/*       
