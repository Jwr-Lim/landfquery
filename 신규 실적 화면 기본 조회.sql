ALTER PROC USP_POP_004_RESULT_NEW_ORDER_QUERY(
...
DECLARE @ST_CHK NVARCHAR(1) = 'N'
IF EXISTS(
	
     SELECT *FROM PD_RESULT A WITH (NOLOCK) 
     INNER JOIN PD_ORDER B WITH (NOLOCK) ON 
     ...
     WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD AND A.PROC_CD = @PROC_CD 
       AND A.EQP_CD = @EQP_CD AND A.S_CHK = 'N' AND A.EDATE IS NULL
)
BEGIN 
	SET @ST_CHK = 'Y'
END 
...
BEGIN 
    SET @EQP_NM = 
        ISNULL((SELECT A.EQP_NM FROM BA_EQP A WITH (NOLOCK) 
        WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.EQP_CD = @EQP_CD AND A.PROC_CD = @PROC_CD),'')

    SELECT TOP 1 @ORDER_NO = A.ORDER_NO, @REVISION = A.REVISION, @PROC_NO = A.PROC_NO, 
    @ORDER_TYPE = A.ORDER_TYPE, @ORDER_FORM = A.ORDER_FORM, @ROUT_NO = A.ROUT_NO, @ROUT_VER = A.ROUT_VER,
    @S_CHK = A.S_CHK, @RESULT_SEQ = A.RESULT_SEQ 
    ...
    FROM PD_RESULT A WITH (NOLOCK) 
     ...
     AND A.ORDER_TYPE = B.ORDER_TYPE AND A.ORDER_FORM = B.ORDER_FORM AND A.WC_CD = B.WC_CD AND A.LINE_CD = B.LINE_CD 
     WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD AND A.PROC_CD = @PROC_CD 
       AND A.EQP_CD = @EQP_CD AND A.S_CHK = 'N' AND (
			A.EDATE IS NULL     AND @ST_CHK = 'Y'
		OR A.EDATE IS NOT NULL AND @ST_CHK = 'N'
		)
	...
	SELECT TOP 1 @USEM_ITEM_CD = B.ITEM_CD 
		FROM PD_RESULT A WITH (NOLOCK) 
		INNER JOIN PD_USEM B WITH (NOLOCK) ON A.DIV_CD = B.DIV_CD AND A.PLANT_CD = B.PLANT_CD AND 
		A.ORDER_NO = B.ORDER_NO AND A.REVISION = B.REVISION AND A.PROC_NO = B.PROC_NO AND A.ORDER_TYPE = B.ORDER_TYPE AND A.ORDER_FORM = B.ORDER_FORM 
    ...
    WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.WC_CD = @WC_CD AND A.LINE_CD = @LINE_CD AND A.PROC_CD = @PROC_CD 
       AND A.EQP_CD = @EQP_CD AND A.S_CHK = 'N' AND A.RESULT_SEQ = @RESULT_SEQ --AND A.EDATE IS NULL
	ORDER BY B.USEM_SEQ 
	...
	CASE WHEN Z.ORDER_NO IS NULL THEN 'D'         
	     WHEN Z.EDATE IS NULL AND Z.DEL_FLG = 'Y' THEN 'S'        
		 WHEN Z.EDATE IS NULL AND Z.DEL_FLG = 'N' THEN 'R'
	 ...
	 ISNULL(Z.EQP_CD, '%') ELSE    
	 ISNULL(Z.EQP_CD,  
	  
	 ISNULL((SELECT TOP 1 AA.EQP_CD FROM PD_RESULT AA WITH (NOLOCK)  	 
     WHERE AA.DIV_CD = @DIV_CD AND AA.PLANT_CD = @PLANT_CD AND AA.WC_CD = @WC_CD AND AA.LINE_CD = @LINE_CD AND AA.PROC_CD = @PROC_CD   
	...
	INNER JOIN PD_RESULT Z WITH (NOLOCK)         
	...
	AND A.WC_CD = Z.WC_CD AND A.LINE_CD = Z.LINE_CD AND B.PROC_CD = Z.PROC_CD AND Z.RESULT_SEQ = @RESULT_SEQ -- AND Z.EDATE IS NULL 
    AND Z.RESULT_SEQ = @RESULT_SEQ AND Z.EQP_CD = @EQP_CD       
...
--	SELECT 'V' AS GROUP_SPEC_CD    
	...
	UPDATE A SET A.DEL_FLG = 'N' FROM PD_RESULT A WITH (NOLOCK) 
	WHERE A.DIV_CD = @DIV_CD AND A.PLANT_CD = @PLANT_CD AND A.ORDER_NO = @ORDER_NO AND A.REVISION = @REVISION    
	  AND A.ORDER_TYPE = @ORDER_TYPE AND A.ORDER_FORM = @ORDER_FORM AND A.ROUT_NO = @ROUT_NO AND A.ROUT_VER = @ROUT_VER    
